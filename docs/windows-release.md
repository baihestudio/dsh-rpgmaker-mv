...
3. Review the missing-prerequisite list and consent when prompted. The installer uses WinGet only after consent for:
...
No Git clone, npm install, or manual package command is needed for this path. Install is per-user and does not require elevation. Re-running `Install.cmd` is the supported repair path; a previous runtime is retained by the staged runtime swap for recovery.
...
Double-click `Uninstall.cmd` in the installed program root. Default uninstall removes program files, app-owned runtimes, and cache, and removes only the BaiheStudio Start Menu shortcut. It preserves DSH state, credentials, logs, recent-project metadata, and all projects (projects are never discovered or deleted by the uninstaller).
