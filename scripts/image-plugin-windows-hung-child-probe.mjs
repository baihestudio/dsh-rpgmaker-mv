...
setTimeout(() => controller.abort('native Windows hung-child probe'), 100)
...
try {
  ...
  if (elapsedMs > IMAGE_OPERATION_CLEANUP_GRACE_MS + 1500) {
    throw new Error(`Windows image cancellation exceeded the five-second cleanup grace (${elapsedMs} ms).`)
  }
  ...
  console.log(JSON.stringify({ ok: true, elapsedMs, code, logPath }))
} finally {
  clearChildSpawner()
  clearTreeTerminator()
  clearTerminationCommandSpawner()
