...
async function prepareUnlocked(options: RpgMakerDeploymentOptions, projectPath: string): Promise<RpgMakerDeployment> {
  ...
  if (dumpedIds.filter((id) => id === DSH_TOOL_TIMEOUT_POLICY_ROW_ID).length > 1) throw new RpgMakerStartupError(`DSH composition validation contained duplicate ${DSH_TOOL_TIMEOUT_POLICY_ROW_ID} rows.`);
...
async function validatePresetComposition(
  ...
  let hostComposition: string;
  try {
    hostComposition = await readFile(compositionPath, 'utf8');
  } catch {
    throw new RpgMakerStartupError(`DSH timeout policy Host composition could not be read: ${compositionPath}`);
  }
  const timeoutRows = timeoutPolicyRows(hostComposition).filter((id) => id === DSH_TOOL_TIMEOUT_POLICY_ROW_ID);
  if (timeoutRows.length !== 1) throw new RpgMakerStartupError(`DSH timeout policy Host composition must contain exactly one ${DSH_TOOL_TIMEOUT_POLICY_ROW_ID} row; found ${timeoutRows.length}.`);
  let result;
  try {
  ...
  if (dumpedIds.filter((id) => id === DSH_TOOL_TIMEOUT_POLICY_ROW_ID).length > 1) {
    throw new RpgMakerStartupError(`DSH composition validation contained duplicate ${DSH_TOOL_TIMEOUT_POLICY_ROW_ID} rows.`);
  }
